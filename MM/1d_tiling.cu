...
__global__ void oned_tiling(int M, int N, int K,
                          const float *A,
                          const float *B,
                          float *C)
{
    ...
    int col = blockIdx.x * BLOCKSIZE + tx; //output row & col
    ...
    for (int phase = 0; phase < CEIL_DIV(K, BLOCKSIZE); phase++) {
        ...
        for (int k = 0; k < BLOCKSIZE; k++) {
            float tmpB = Bs[k][tx];
            for(int i=0;i<TM;i++) {
                tmp[i] += As[ty + i * BLOCKSIZE][k] * tmpB;
            }
        }
...
float run_one_size(int max_size)
{
    ...
    dim3 gridDim(CEIL_DIV(N, BLOCKSIZE), CEIL_DIV(M, BLOCKSIZE * TM));
