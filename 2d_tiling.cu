...
__global__ void __launch_bounds__((BM * BN) / (TM * TN), 1) twod_tiling(int M, int N, int K, float* A, float *B, float *C) 
{
    const int cRow = blockIdx.y;
    const int cCol = blockIdx.x;

    constexpr int nothreads = (BM * BN) / (TM * TN);

    const int cThreadRow = threadIdx.x / (BN / TN);
    const int cThreadCol = threadIdx.x % (BN / TN);
    ...
    constexpr int strideA = nothreads / BK;
    constexpr int strideB = nothreads / BN;
    ...
    for(int bIdx = 0; bIdx < K; bIdx += BK) {
        ...
        for(int offset = 0; offset < BM; offset += strideA) {
            As[(aThreadRow + offset) * BK + aThreadCol] = A[(aThreadRow + offset) * K + aThreadCol];
        }

        for(int offset = 0; offset < BK; offset += strideB) {
            Bs[(bThreadRow + offset) * BN + bThreadCol] = B[(bThreadRow + offset) * N + bThreadCol];
        }
        ...
        for(int k = 0; k < BK; k++) {
            ...
            for(int i = 0;i<TM;i++) {
                regM[i] = As[(cThreadRow * TM + i) * BK + k];
            }

            for(int j = 0; j < TN; j++) {
                regN[j] = Bs[k * BN + (cThreadCol * TN + j)];
            }

            for(int ti = 0; ti < TM; ti++) {
                for(int tj =0; tj < TN; tj++) {
                    tmp[ti * TN + tj] += regM[ti] * regN[tj];
                }
            }
        }
        __syncthreads();
    ...
    for(int ti = 0; ti < TM; ti++) {
        for(int tj =0; tj < TN; tj++) {
            C[(cThreadRow * TM + ti) * N + (cThreadCol * TN + tj)] = tmp[ti * TN + tj];
        }
    }
}
...
int main(int argc, char **argv) {
   ...
   // printDeviceProperties();
    ...
    twod_tiling<128, 128, 8, 8, 8><<<gridDim, blockDim>>>(M, N, K, dA, dB, dC);
