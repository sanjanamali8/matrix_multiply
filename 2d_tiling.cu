...
__global__ void twod_tiling(int M, int N, int K,
                          const float *A,
                          const float *B,
                          float *C)
{
    __shared__ float As[BLOCKSIZE * TM][BLOCKSIZE];
    ...
    for (int phase = 0; phase < CEIL_DIV(K, BLOCKSIZE); phase++) {
        ...
        for (int i = 0; i < TM; i++) {
            ...
            if (load_row < M && a_col < K)
                As[ty + i * BLOCKSIZE][tx] = A[load_row * K + a_col];
            else
                As[ty + i * BLOCKSIZE][tx] = 0.0f;
        }

        for(int i=0;i<TN;i++) {
            int load_col = b_col + i * BLOCKSIZE;
            if (b_row < K && load_col < N)
                Bs[ty][tx + i * BLOCKSIZE] = B[b_row * N + load_col];
            else
                Bs[ty][tx + i * BLOCKSIZE] = 0.0f;
        }
        ...
        for (int k = 0; k < BLOCKSIZE; k++) {
            for(int i=0;i<TM;i++) {
                float tmpA = As[ty + i * BLOCKSIZE][k];
                for(int j = 0;j<TN;j++) {
                    tmp[i][j] +=  tmpA * Bs[k][tx + j * BLOCKSIZE];
                }
            }
        }
...
float run_one_size(int max_size)
{
    ...
    twod_tiling<<<gridDim, blockDim>>>(M, N, K, dA, dB, dC);
