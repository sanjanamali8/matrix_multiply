...
__global__ void smem_gemm(int M, int N, int K, float* A, float *B, float *C) 
{
    int cRow = blockIdx.x;
    int cCol = blockIdx.y;

    int localThreadRow = threadIdx.x / BLOCK;
    int localThreadCol = threadIdx.x % BLOCK;
    ...
    float tmp = 0.0f;
    for(int bIdx = 0; bIdx < K; bIdx += BLOCK) {

        As[localThreadRow * BLOCK + localThreadCol] = A[localThreadRow * K + localThreadCol];
        Bs[localThreadRow * BLOCK + localThreadCol] = B[localThreadRow * N + localThreadCol];
        ...
        for(int k = 0; k < BLOCK; k++) {
            tmp += As[localThreadRow * BLOCK + k] * Bs[k * BLOCK + localThreadCol];
        }
        __syncthreads();
    }
    C[localThreadRow * N + localThreadCol] = tmp;
}
